...
def create_app():
    ...
    # Blueprint de Klap Checkout FLEX - ELIMINADO
    # Ya no se registra el blueprint de Klap
    ...
    def inject_shift_info():
        ...
        try:
            ...
            from flask import current_app
            # Obtener CHILE_TZ de forma segura
            try:
                CHILE_TZ = current_app.config.get('CHILE_TZ')
                if CHILE_TZ is None:
                    # Intentar importar desde el módulo
                    import app
                    CHILE_TZ = getattr(app, 'CHILE_TZ', None)
                if CHILE_TZ is None:
                    # Fallback: crear directamente
                    CHILE_TZ = pytz.timezone('America/Santiago')
            except Exception as tz_error:
                current_app.logger.warning(f"Error al obtener CHILE_TZ en context processor: {tz_error}")
                CHILE_TZ = pytz.timezone('America/Santiago')
